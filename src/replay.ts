...
async function runReplayBody(eventId: string, config: Config, speed: number, logger: MatchLogger): Promise<void> {
  ...
  await logger.stream(renderEventLines(kickoff, running, strings), STREAM_GAP_MS);
  ...
  for (const e of events) {
    ...
    await logger.stream(renderEventLines(e, running, strings), STREAM_GAP_MS);
  }
...
async function pacedGap(gapMs: number, logger: MatchLogger, strings: Strings): Promise<void> {
  let remaining = gapMs;
  let first = true;
  while (remaining > 0) {
